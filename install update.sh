...
# (c) DJ0ABR
# Unified update installer for G4KLX stack on Debian x86_64 (MMDVMHost, YSFGateway, ircDDBGateway, DMRGateway)
# Goals: idempotent, fail-fast, minimal duplication, small functions, pauses between blocks for debugging.
# Usage: sudo ./install_update.sh [--no-pause]

# this script does NOT install or update the G4KLX modules or the settings
# in updates ONLY the DJ0ABR extensions without modifying any settings
...
systemd_enable_and_restart_all() {
  ...
  systemctl restart mmdvmhost || true
  systemctl restart ysfgateway || true
  systemctl restart ircddbgateway || true
  systemctl restart dmrgateway || true
  systemctl restart mmdvm-status.service || true
  systemctl restart mmdvm-DVconfig.service || true
...
main() {
  ...
  need_root
  log "Checking and updateing base system..."
  base_packages
  remove_modemmanager
  ensure_user_mmdvm
  ensure_dirs
  ...
  echo "New Software installed and running, Reboot usually not required." 
}
